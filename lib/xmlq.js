...
module.exports = {
     ...
     */
    findChild(node, name) {
        const prefix = node.attributes ? Object.keys(node.attributes).reduce((result, key) => {
          if (node.attributes[key] === 'http://schemas.openxmlformats.org/spreadsheetml/2006/main') {
            const [, prefix] = key.split(':')
            return prefix || ''
          }
          return result
        }, '') : ''
        return _.find(node.children, { name: prefix ? (prefix + ':' + name) : name }) || _.find(node.children, { name });
    },
    /**
